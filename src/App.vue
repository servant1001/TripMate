...
import type { AlbumPhoto, Booking, BookingType, Expense, ExpenseKind, ExpenseSplitMode, Favorite, FavoriteType, ItineraryActivityKind, ItineraryItem, Member, PackingItem, PaymentMethod, PaymentTool, PaymentToolType, PaymentToolVisibility, PaymentTransaction, RewardCapPeriod, RewardRule, RewardType, Role, Settlement, ShoppingItem, ShoppingPriority, ShoppingStatus, ShoppingType, TodoItem, TodoScope, TravelInsurance, Trip } from './types'
...
const activeTripTab = computed<TripTab>(() => {
  if (route.name === 'trip-insurance') return 'insurance'
  if (route.name === 'trip-payment-tools') return 'payments'
  const tab = route.query.tab
  return tab === 'itinerary' || tab === 'map' || tab === 'expenses' || tab === 'todos' || tab === 'packing' || tab === 'bookings' || tab === 'favorites' || tab === 'album' || tab === 'shopping' || tab === 'insurance' || tab === 'payments' || tab === 'members' ? tab : 'overview'
})
...
function selectTripTab(tab: TripTab) {
  ...
  delete query.tab
  if (tab === 'insurance') { void router.push({ name: 'trip-insurance', params: { tripId: activeId.value }, query }); return }
  if (tab === 'payments') { void router.push({ name: 'trip-payment-tools', params: { tripId: activeId.value }, query }); return }
  void router.push({ name: 'trip-dashboard', params: { tripId: activeId.value }, query: tab === 'overview' ? query : { ...query, tab } })
}
function syncRoute() { const name = String(route.name || 'trips'); if (name === 'login' || name === 'register' || name === 'forgot-password') { screen.value = 'login'; authMode.value = name === 'register' ? 'register' : 'login'; return } if (name === 'profile') { screen.value = 'profile'; if (user.value) profile.displayName = user.value.displayName || user.value.email?.split('@')[0] || ''; return } if (name === 'trip-dashboard' || name === 'trip-insurance' || name === 'trip-payment-tools') { activeId.value = String(route.params.tripId); screen.value = 'trip'; return } screen.value = 'trips'; showCreate.value = name === 'trip-create' }
...
<TripItineraryCard v-if="activeTripTab === 'overview' || activeTripTab === 'itinerary'" :days="itineraryDays" :personal-items="currentPersonalItems" :shopping-items="currentShoppingItems" :can-edit-trip="canEditTrip" :sorting-enabled="itinerarySortingEnabled" :format-date="formatItineraryDate" :duration="itineraryDuration" :time-warning="itineraryTimeWarning" :maps-url="mapsUrl" @add="openNewItemForm" @add-after="openItemFormAfter" @add-personal="openPersonalItemForm" @toggle="toggleItinerary" @edit="openItemFormForEdit" @remove="removeItem" @create-group="openItineraryGroupForm($event.entries)" @edit-group="openItineraryGroupForm([], $event)" @dissolve-group="dissolveItineraryGroup" @toggle-sorting="toggleItinerarySorting" @sort="sortItineraryItems" @sort-group="sortGroupItineraryItems" @sort-personal="sortPersonalItineraryItems" @move="moveItineraryItem" />
        ...
        <TripPaymentToolsCard v-if="activeTripTab === 'payments'" :trip="current" :tools="currentPaymentTools" :rules="currentRewardRules" :transactions="currentPaymentTransactions" :balances="currentStoredBalances" :user-id="user?.uid || current.ownerId" :can-edit="canEditTrip" :member-name="memberName" @add-tool="openPaymentToolForm()" @edit-tool="openPaymentToolForm" @remove-tool="removePaymentTool" @toggle-tool="togglePaymentTool" @add-rule="openRewardRuleForm" @edit-rule="openRewardRuleForm(undefined, $event)" @remove-rule="removeRewardRule" @add-transaction="openPaymentTransactionForm" @edit-transaction="openPaymentTransactionForm(undefined, $event)" @remove-transaction="removePaymentTransaction" @manage-balance="openStoredBalanceForm" />
        <TripPaymentRecommendationCard v-if="activeTripTab === 'payments'" :trip="current" :tools="currentPaymentTools" :rules="currentRewardRules" :user-id="user?.uid || current.ownerId" />
        <TripPaymentSharedToolsCard v-if="activeTripTab === 'payments'" :summaries="currentPaymentToolSummaries" :user-id="user?.uid || current.ownerId" :member-name="memberName" />
...
<el-form-item label="相關網址（選填）"><el-input v-model="todo.url" placeholder="貼上預訂、文件或參考連結" /><small>可直接貼上網址，儲存後可從待辦卡片開啟。</small></el-form-item>
<el-form-item label="備註（選填）"><el-input v-model="todo.note" type="textarea" :rows="3" maxlength="240" show-word-limit placeholder="例如：準備內容、聯絡資訊或注意事項" /></el-form-item>
</el-form>
...
<template #footer><el-button :disabled="savingDailyBudget" @click="showDailyBudget=false">取消</el-button><el-button type="primary" :loading="savingDailyBudget" :disabled="savingDailyBudget" @click="saveDailyBudget">儲存每日預算</el-button></template>
  ...
  <el-dialog v-model="showPaymentTool" :title="editingPaymentToolId ? '編輯支付工具' : '新增支付工具'" width="min(94vw, 620px)">
    <el-form label-position="top">
      <div class="two-col"><el-form-item label="工具名稱" required><el-input v-model="paymentTool.name" placeholder="例如：富邦 J 卡、Suica" /></el-form-item><el-form-item label="類型"><el-select v-model="paymentTool.type"><el-option label="信用卡" value="credit_card"/><el-option label="簽帳金融卡" value="debit_card"/><el-option label="電子支付" value="electronic_payment"/><el-option label="交通卡" value="transport_card"/><el-option label="現金" value="cash"/><el-option label="其他" value="other"/></el-select></el-form-item><el-form-item label="發卡／服務單位"><el-input v-model="paymentTool.issuer" /></el-form-item><el-form-item label="末四碼（選填）"><el-input v-model="paymentTool.lastFourDigits" maxlength="4" inputmode="numeric" /></el-form-item><el-form-item label="結算幣別"><el-input v-model="paymentTool.settlementCurrency" /></el-form-item><el-form-item label="海外手續費（%）"><el-input-number v-model="paymentTool.foreignTransactionFeeRatePercent" :min="0" :precision="2" controls-position="right" /></el-form-item></div>
      <el-form-item label="工具圖片網址（選填）"><el-input v-model="paymentToolImageUrl" placeholder="https://example.com/card-image.png" /><p class="payment-image-hint">支援可公開存取的圖片網址；儲存後會顯示在工具名稱旁。</p><img v-if="paymentToolImageUrl" :src="paymentToolImageUrl" class="payment-image-preview" alt="支付工具圖片預覽" /></el-form-item>
      <el-form-item label="可見性"><el-radio-group v-model="paymentTool.visibility"><el-radio-button label="private">僅自己</el-radio-button><el-radio-button label="summary">成員看摘要</el-radio-button><el-radio-button label="trip_members">旅行成員</el-radio-button></el-radio-group></el-form-item><el-form-item label="備註"><el-input v-model="paymentTool.note" type="textarea" /></el-form-item>
    </el-form><template #footer><el-button @click="showPaymentTool=false">取消</el-button><el-button type="primary" @click="savePaymentTool">儲存工具</el-button></template>
  </el-dialog>
  <el-dialog v-model="showRewardRule" :title="editingRewardRuleId ? '編輯回饋規則' : '新增回饋規則'" width="min(94vw, 680px)"><el-form label-position="top"><div class="two-col"><el-form-item label="回饋規則名稱" required><el-input v-model="rewardRule.name" /></el-form-item><el-form-item label="支付工具"><el-select v-model="rewardRule.paymentToolId"><el-option v-for="tool in currentPaymentTools.filter((item) => item.ownerUserId === (user?.uid || current?.ownerId))" :key="tool.id" :label="tool.name" :value="tool.id" /></el-select></el-form-item><el-form-item label="基本回饋（%）"><el-input-number v-model="rewardRule.baseRatePercent" :min="0" :precision="2" controls-position="right" /></el-form-item><el-form-item label="加碼回饋（%）"><el-input-number v-model="rewardRule.bonusRatePercent" :min="0" :precision="2" controls-position="right" /></el-form-item><el-form-item label="回饋金額上限"><el-input-number v-model="rewardRule.rewardCap" :min="0" controls-position="right" /></el-form-item><el-form-item label="符合回饋消費上限"><el-input-number v-model="rewardRule.maximumEligibleSpend" :min="0" controls-position="right" /></el-form-item><el-form-item label="活動開始"><el-date-picker v-model="rewardRule.periodStart" type="date" value-format="YYYY-MM-DD" /></el-form-item><el-form-item label="活動結束"><el-date-picker v-model="rewardRule.periodEnd" type="date" value-format="YYYY-MM-DD" /></el-form-item></div><el-form-item><el-checkbox v-model="rewardRule.requiresRegistration">需要活動登錄</el-checkbox><el-checkbox v-if="rewardRule.requiresRegistration" v-model="rewardRule.registrationCompleted">已完成登錄</el-checkbox></el-form-item><el-form-item label="備註"><el-input v-model="rewardRule.note" type="textarea" /></el-form-item></el-form><template #footer><el-button @click="showRewardRule=false">取消</el-button><el-button type="primary" @click="saveRewardRule">儲存規則</el-button></template></el-dialog>
  <el-dialog v-model="showPaymentTransaction" :title="editingPaymentTransactionId ? '編輯付款紀錄' : '新增付款紀錄'" width="min(94vw, 680px)"><el-form label-position="top"><div class="two-col"><el-form-item label="支付工具" required><el-select v-model="paymentTransaction.paymentToolId"><el-option v-for="tool in currentPaymentTools.filter((item) => item.ownerUserId === (user?.uid || current?.ownerId))" :key="tool.id" :label="tool.name" :value="tool.id" /></el-select></el-form-item><el-form-item label="消費名稱" required><el-input v-model="paymentTransaction.title" /></el-form-item><el-form-item label="店家"><el-input v-model="paymentTransaction.merchant" /></el-form-item><el-form-item label="分類"><el-input v-model="paymentTransaction.category" /></el-form-item><el-form-item label="日期" required><el-date-picker v-model="paymentTransaction.transactionDate" type="date" value-format="YYYY-MM-DD" /></el-form-item><el-form-item label="原始金額" required><el-input-number v-model="paymentTransaction.originalAmount" :min="0" controls-position="right" /></el-form-item><el-form-item label="原始幣別"><el-input v-model="paymentTransaction.originalCurrency" /></el-form-item><el-form-item label="匯率"><el-input-number v-model="paymentTransaction.exchangeRate" :min="0.000001" :precision="6" controls-position="right" /></el-form-item><el-form-item label="交易類型"><el-select v-model="paymentTransaction.transactionType"><el-option label="消費" value="purchase"/><el-option label="儲值" value="top_up"/><el-option label="退款" value="refund"/><el-option label="調整" value="adjustment"/></el-select></el-form-item><el-form-item label="狀態"><el-select v-model="paymentTransaction.status"><el-option label="已入帳" value="posted"/><el-option label="待入帳" value="pending"/><el-option label="部分退款" value="partially_refunded"/><el-option label="已退款" value="refunded"/><el-option label="已取消" value="cancelled"/></el-select></el-form-item></div><el-form-item label="退款金額（選填）"><el-input-number v-model="paymentTransaction.refundedAmount" :min="0" :max="paymentTransaction.originalAmount" controls-position="right" /></el-form-item><el-form-item><el-checkbox v-model="paymentTransaction.syncExpense">同步建立個人旅行開銷（僅新交易）</el-checkbox></el-form-item><el-form-item label="備註"><el-input v-model="paymentTransaction.note" type="textarea" /></el-form-item></el-form><template #footer><el-button @click="showPaymentTransaction=false">取消</el-button><el-button type="primary" @click="savePaymentTransaction">儲存付款紀錄</el-button></template></el-dialog>
