...
function normalizeFavoriteType(favoriteType: FavoriteType): FavoriteType { return favoriteType === 'cafe' ? 'restaurant' : favoriteType === 'shop' || favoriteType === 'alternative' || favoriteType === 'other' ? 'attraction' : favoriteType }
function favoriteToItineraryType(favoriteType: FavoriteType) { return ({ attraction: '景點', restaurant: '餐廳', transport: '交通', stay: '住宿', cafe: '餐廳', shop: '景點', alternative: '景點', other: '景點' } as Record<FavoriteType, string>)[favoriteType] }
function favoriteTypeLabel(favoriteType: FavoriteType) { return ({ attraction: '景點', restaurant: '餐廳', transport: '交通', stay: '住宿', cafe: '餐廳', shop: '景點', alternative: '景點', other: '景點' } as Record<FavoriteType, string>)[favoriteType] }
const favoritePickerOptions: Array<{ value: FavoriteType | 'all'; label: string }> = [{ value: 'all', label: '全部' }, { value: 'attraction', label: '景點' }, { value: 'restaurant', label: '餐廳' }, { value: 'transport', label: '交通' }, { value: 'stay', label: '住宿' }]
...
<el-form-item v-if="item.type === '交通'" label="抵達站／目的地（選填）">
...
<el-form label-position="top">
<div class="two-col"><el-form-item label="收藏類型"><el-select v-model="favorite.type"><el-option label="景點" value="attraction" /><el-option label="餐廳" value="restaurant" /><el-option label="交通" value="transport" /><el-option label="住宿" value="stay" /></el-select><small>類型與每日行程一致，加入行程時會直接帶入。</small></el-form-item><el-form-item label="名稱"><el-input v-model="favorite.name" placeholder="例如：淺草寺" /></el-form-item></div>
