...
import type { AlbumPhoto, Booking, BookingType, Expense, ExpenseKind, Favorite, FavoriteType, ItineraryItem, Member, PackingItem, Role, Settlement, TodoItem, TodoScope, Trip } from './types'
...
const router = useRouter(); const route = useRoute(); const store = useTripStore(); const activeId = ref(''); const screen = ref<'trips'|'trip'|'login'|'profile'>('trips'); const authResolving = ref(firebaseEnabled && Boolean(auth)); const showCreate = ref(false); const showEdit = ref(false); const showJoin = ref(false); const showMember = ref(false); const showItem = ref(false); const showExpense = ref(false); const showTodo = ref(false); const showPacking = ref(false); const showBooking = ref(false); const showFavorite = ref(false); const showAlbum = ref(false); const savingAlbum = ref(false); const showPersonalBudget = ref(false); const savingPersonalBudget = ref(false); const showCategoryBudgets = ref(false); const savingCategoryBudgets = ref(false)
type TripTab = 'overview' | 'itinerary' | 'map' | 'expenses' | 'todos' | 'packing' | 'bookings' | 'favorites' | 'album' | 'members'
const current = computed(() => store.trip(activeId.value)); const currentItems = computed(() => store.items(activeId.value)); const currentExpenses = computed(() => store.tripExpenses(activeId.value)); const currentTodos = computed(() => store.tripTodos(activeId.value)); const currentPackingItems = computed(() => store.tripPackingItems(activeId.value)); const currentBookings = computed(() => store.tripBookings(activeId.value)); const currentFavorites = computed(() => store.tripFavorites(activeId.value)); const currentAlbumPhotos = computed(() => store.tripAlbumPhotos(activeId.value)); const currentSettlements = computed(() => store.tripSettlements(activeId.value));
...
const activeTripTab = computed<TripTab>(() => {
  ...
  return tab === 'itinerary' || tab === 'map' || tab === 'expenses' || tab === 'todos' || tab === 'packing' || tab === 'bookings' || tab === 'favorites' || tab === 'album' || tab === 'members' ? tab : 'overview'
})
...
async function movePackingItem(item: PackingItem, target: PackingItem) { if (!canEditTrip.value) return; const group = currentPackingItems.value.filter((entry) => entry.isShared === item.isShared).sort((a, b) => (a.order ?? Number.MAX_SAFE_INTEGER) - (b.order ?? Number.MAX_SAFE_INTEGER) || a.createdAt - b.createdAt); const itemIndex = group.findIndex((entry) => entry.id === item.id); const targetIndex = group.findIndex((entry) => entry.id === target.id); if (itemIndex < 0 || targetIndex < 0 || itemIndex === targetIndex) return; [group[itemIndex], group[targetIndex]] = [group[targetIndex], group[itemIndex]]; try { await store.reorderPackingItems(group) } catch (error) { ElMessage.error(error instanceof Error ? error.message : '無法更新行李排序。') } }
...
<el-button type="primary" @click="saveProfile">儲存個人資料</el-button>
    ...
    <section v-else-if="current" class="page trip-detail-page">
      ...
      <div class="trip-detail-layout" :class="{ 'is-single-detail': activeTripTab !== 'overview' }" role="tabpanel" :aria-label="activeTripTab === 'overview' ? '旅行總覽' : activeTripTab === 'itinerary' ? '行程' : activeTripTab === 'map' ? '地圖' : activeTripTab === 'expenses' ? '開銷' : activeTripTab === 'todos' ? '待辦' : activeTripTab === 'packing' ? '行李' : activeTripTab === 'bookings' ? '預訂' : activeTripTab === 'favorites' ? '收藏' : activeTripTab === 'album' ? '旅行相簿' : '旅伴與結算'">
        ...
        <TripPackingCard v-if="activeTripTab === 'packing'" :trip="current" :items="currentPackingItems" :can-edit-trip="canEditTrip" :member-name="memberName" @add="openPackingForm()" @toggle="togglePackingItem" @move="movePackingItem" @edit="openPackingForm" @remove="removePackingItem" />
