...
import type { Expense, ExpenseKind, ItineraryItem, Member, Role, Settlement, TodoItem, Trip } from './types'
...
const router = useRouter(); const route = useRoute(); const store = useTripStore(); const activeId = ref(''); const screen = ref<'trips'|'trip'|'login'|'profile'>('trips'); const authResolving = ref(firebaseEnabled && Boolean(auth)); const showCreate = ref(false); const showEdit = ref(false); const showJoin = ref(false); const showMember = ref(false); const showItem = ref(false); const showExpense = ref(false); const showTodo = ref(false); const showPersonalBudget = ref(false); const savingPersonalBudget = ref(false); const showCategoryBudgets = ref(false); const savingCategoryBudgets = ref(false)
type TripTab = 'overview' | 'itinerary' | 'expenses' | 'todos' | 'members'
const current = computed(() => store.trip(activeId.value)); const currentItems = computed(() => store.items(activeId.value)); const currentExpenses = computed(() => store.tripExpenses(activeId.value)); const currentTodos = computed(() => store.tripTodos(activeId.value)); const currentSettlements = computed(() => store.tripSettlements(activeId.value));
...
const activeTripTab = computed<TripTab>(() => {
  ...
  return tab === 'itinerary' || tab === 'expenses' || tab === 'todos' || tab === 'members' ? tab : 'overview'
})
...
function openTodoForm(existing?: TodoItem) { if (!canEditTrip.value) return ElMessage.warning('Viewer 僅能查看待辦，無法修改。'); editingTodoId.value = existing?.id || null; Object.assign(todo, existing ? { title: existing.title, assigneeId: existing.assigneeId || '', dueDate: existing.dueDate || '' } : { title: '', assigneeId: '', dueDate: '' }); showTodo.value = true }
async function saveTodo() { if (!current.value || !todo.title.trim()) return ElMessage.warning('請填寫待辦事項。'); try { const payload = { tripId: current.value.id, title: todo.title.trim(), assigneeId: todo.assigneeId || '', dueDate: todo.dueDate || '' }; const existing = editingTodoId.value ? currentTodos.value.find((entry) => entry.id === editingTodoId.value) : undefined; if (existing) await store.updateTodo({ ...existing, ...payload }); else await store.addTodo(payload); showTodo.value = false; editingTodoId.value = null; ElMessage.success('待辦已儲存。') } catch (error) { ElMessage.error(error instanceof Error ? error.message : '無法儲存待辦。') } }
...
<el-button type="primary" @click="saveProfile">儲存個人資料</el-button>
    ...
    <section v-else-if="current" class="page trip-detail-page">
      ...
      <div class="trip-detail-layout" :class="{ 'is-single-detail': activeTripTab !== 'overview' }" role="tabpanel" :aria-label="activeTripTab === 'overview' ? '旅行總覽' : activeTripTab === 'itinerary' ? '行程' : activeTripTab === 'expenses' ? '開銷' : activeTripTab === 'todos' ? '待辦' : '旅伴與結算'">
...
<div class="two-col">
<el-form-item label="負責旅伴（選填）"><el-select v-model="todo.assigneeId" clearable placeholder="尚未指派"><el-option v-for="m in current?.members" :key="m.id" :label="m.name" :value="m.id" /></el-select></el-form-item>
