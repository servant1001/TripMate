...
function selectShoppingItinerary(entry: ItineraryItem) { const selected = new Set(shoppingItineraryItemIds.value); if (selected.has(entry.id)) selected.delete(entry.id); else selected.add(entry.id); shoppingItineraryItemIds.value = [...selected] }
function confirmShoppingItinerarySelection() { showShoppingItineraryPicker.value = false }
function clearShoppingItinerary() { shoppingItineraryItemIds.value = []; shopping.itineraryItemId = ''; showShoppingItineraryPicker.value = false }
...
<div class="shopping-itinerary-picker-list"><button v-for="entry in shoppingItineraryEntries" :key="entry.id" type="button" class="shopping-itinerary-picker-row" :class="{ 'is-selected': shoppingItineraryItemIds.includes(entry.id) }" @click="selectShoppingItinerary(entry)"><img v-if="entry.imageUrl" :src="entry.imageUrl" :alt="`${entry.title} 圖片`" /><span v-else class="shopping-itinerary-picker-placeholder">{{ (entry.type || '行程').slice(0, 1) }}</span><span class="shopping-itinerary-picker-row-copy"><strong>{{ entry.title }}</strong><span><time>{{ entry.time || '未排時間' }}</time><template v-if="entry.endTime">－{{ entry.endTime }}</template><em>{{ entry.activityKind === 'personal' ? '個人行程' : entry.type }}</em></span><small v-if="entry.location">{{ entry.location }}</small></span><span class="shopping-itinerary-picker-select">{{ shoppingItineraryItemIds.includes(entry.id) ? '已勾選' : '選擇' }}</span></button></div>
</div>
<div v-else class="shopping-itinerary-picker-empty"><strong>尚未建立可關聯的行程</strong><p>先在每日行程中建立行程後，再回來關聯商品。</p></div>
<template #footer><el-button @click="showShoppingItineraryPicker=false">取消</el-button><el-button v-if="shoppingItineraryItemIds.length" @click="clearShoppingItinerary">清除關聯</el-button><el-button type="primary" @click="confirmShoppingItinerarySelection">確認關聯{{ shoppingItineraryItemIds.length ? `（${shoppingItineraryItemIds.length}）` : '' }}</el-button></template>
  </el-dialog>
