...
import type { AlbumPhoto, Booking, BookingType, Expense, ExpenseKind, ExpenseSplitMode, Favorite, FavoriteType, ItineraryActivityKind, ItineraryItem, Member, PackingItem, Role, Settlement, ShoppingItem, ShoppingPriority, ShoppingStatus, ShoppingType, TodoItem, TodoScope, TravelInsurance, Trip } from './types'
...
type TripTab = 'overview' | 'itinerary' | 'map' | 'expenses' | 'todos' | 'packing' | 'bookings' | 'favorites' | 'album' | 'shopping' | 'insurance' | 'members'
...
const activeTripTab = computed<TripTab>(() => {
  ...
  return tab === 'itinerary' || tab === 'map' || tab === 'expenses' || tab === 'todos' || tab === 'packing' || tab === 'bookings' || tab === 'favorites' || tab === 'album' || tab === 'shopping' || tab === 'insurance' || tab === 'members' ? tab : 'overview'
})
const tripTabLabels: Record<TripTab, string> = { overview: '總覽', itinerary: '行程', map: '地圖', expenses: '開銷', todos: '待辦', packing: '行李', bookings: '預訂', favorites: '收藏', album: '相簿', shopping: '購物', insurance: '保險', members: '旅伴與結算' }
const tripTabOptions: TripTab[] = ['overview', 'itinerary', 'expenses', 'todos', 'packing', 'bookings', 'favorites', 'shopping', 'insurance', 'album', 'map', 'members']
