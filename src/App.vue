...
const itemActivityKind = ref<'shared' | 'free' | 'personal'>('shared'); const personalActivityParentId = ref('')
...
function openItemFormAfter(entry: ItineraryItem) { itemActivityKind.value = 'shared'; personalActivityParentId.value = ''; openItemForm(); insertAfterItemId.value = entry.id; item.date = entry.date }
...
async function duplicateFavorite(favoriteItem: Favorite) { if (!current.value || !canEditTrip.value) return ElMessage.warning('Viewer 僅能查看旅遊收藏，無法修改。'); try { await store.addFavorite({ tripId: current.value.id, name: `${favoriteItem.name}（副本）`, type: favoriteItem.type, location: favoriteItem.location || '', mapUrl: favoriteItem.mapUrl || '', website: favoriteItem.website || '', imageUrl: favoriteItem.imageUrl || '', estimatedCost: favoriteItem.estimatedCost || 0, recommendedBy: favoriteItem.recommendedBy || '', note: favoriteItem.note || '', order: Date.now(), createdBy: user.value?.uid || current.value.ownerId }); ElMessage.success('已複製收藏，可再編輯副本。') } catch (error) { ElMessage.error(error instanceof Error ? error.message : '無法複製旅遊收藏。') } }
...
const selectedShoppingItinerary = computed(() => currentItems.value.find((entry) => entry.id === shopping.itineraryItemId))
...
function openShoppingItineraryPicker() { shoppingItineraryPickerDay.value = selectedShoppingItinerary.value?.date || shoppingItineraryDays.value[0]?.date || ''; showShoppingItineraryPicker.value = true }
function selectShoppingItinerary(entry: ItineraryItem) { shopping.itineraryItemId = entry.id; showShoppingItineraryPicker.value = false }
function clearShoppingItinerary() { shopping.itineraryItemId = ''; showShoppingItineraryPicker.value = false }
...
<el-button type="primary" @click="saveProfile">儲存個人資料</el-button>
    ...
    <section v-else-if="current" class="page trip-detail-page">
      ...
      <div class="trip-detail-layout" :class="{ 'is-single-detail': activeTripTab !== 'overview' }" role="tabpanel" :aria-label="activeTripTab === 'overview' ? '旅行總覽' : activeTripTab === 'itinerary' ? '行程' : activeTripTab === 'map' ? '地圖' : activeTripTab === 'expenses' ? '開銷' : activeTripTab === 'todos' ? '待辦' : activeTripTab === 'packing' ? '行李' : activeTripTab === 'bookings' ? '預訂' : activeTripTab === 'favorites' ? '收藏' : activeTripTab === 'album' ? '旅行相簿' : activeTripTab === 'shopping' ? '購物清單' : '旅伴與結算'">
        <TripItineraryCard v-if="activeTripTab === 'overview' || activeTripTab === 'itinerary'" :days="itineraryDays" :personal-items="currentPersonalItems" :shopping-items="currentShoppingItems" :can-edit-trip="canEditTrip" :sorting-enabled="itinerarySortingEnabled" :format-date="formatItineraryDate" :duration="itineraryDuration" :time-warning="itineraryTimeWarning" :maps-url="mapsUrl" @add="openNewItemForm" @add-after="openItemFormAfter" @add-personal="openPersonalItemForm" @toggle="toggleItinerary" @edit="openItemFormForEdit" @remove="removeItem" @toggle-sorting="toggleItinerarySorting" @sort="sortItineraryItems" @sort-personal="sortPersonalItineraryItems" @move="moveItineraryItem" />
        ...
        <TripShoppingCard v-if="activeTripTab === 'shopping'" :trip="current" :items="currentShoppingItems" :can-edit-trip="canEditTrip" :member-name="memberName" @add="openShoppingForm()" @edit="openShoppingForm" @duplicate="duplicateShoppingItem" @remove="removeShoppingItem" @status="updateShoppingStatus" @convert="convertShoppingToExpense" />
...
<div class="two-col"><el-form-item label="預計購買日期（選填）"><el-date-picker v-model="shopping.plannedDate" type="date" value-format="YYYY-MM-DD" placeholder="選擇日期" /></el-form-item><el-form-item label="關聯行程（選填）"><div class="shopping-itinerary-picker-control"><div class="shopping-itinerary-picker-copy"><strong>{{ selectedShoppingItinerary ? selectedShoppingItinerary.title : '尚未關聯行程' }}</strong><span>{{ selectedShoppingItinerary ? `${selectedShoppingItinerary.date}・${selectedShoppingItinerary.time || '未排時間'}` : '先選擇第幾天，再從該日行程中選取' }}</span></div><div class="shopping-itinerary-picker-actions"><el-button v-if="shopping.itineraryItemId" text class="shopping-itinerary-clear" @click="clearShoppingItinerary">清除</el-button><el-button class="shopping-itinerary-picker-button" @click="openShoppingItineraryPicker">{{ shopping.itineraryItemId ? '更換行程' : '選擇行程' }}</el-button></div></div></el-form-item></div>
...
<template #footer><el-button :disabled="savingShopping" @click="showShopping=false">取消</el-button><el-button type="primary" :loading="savingShopping" :disabled="savingShopping" @click="saveShoppingItem">{{ editingShoppingId ? '儲存變更' : '新增商品' }}</el-button></template>
  ...
  <el-dialog v-model="showShoppingItineraryPicker" title="選擇關聯行程" class="shopping-itinerary-picker-dialog" width="min(92vw, 620px)" append-to-body>
...
<div class="shopping-itinerary-picker-heading"><strong>{{ formatItineraryDate(shoppingItineraryPickerDay) }}</strong><span>{{ shoppingItineraryEntries.length }} 個可選行程</span></div>
<div class="shopping-itinerary-picker-list"><button v-for="entry in shoppingItineraryEntries" :key="entry.id" type="button" class="shopping-itinerary-picker-row" :class="{ 'is-selected': shopping.itineraryItemId === entry.id }" @click="selectShoppingItinerary(entry)"><img v-if="entry.imageUrl" :src="entry.imageUrl" :alt="`${entry.title} 圖片`" /><span v-else class="shopping-itinerary-picker-placeholder">{{ (entry.type || '行程').slice(0, 1) }}</span><span class="shopping-itinerary-picker-row-copy"><strong>{{ entry.title }}</strong><span><time>{{ entry.time || '未排時間' }}</time><template v-if="entry.endTime">－{{ entry.endTime }}</template><em>{{ entry.activityKind === 'personal' ? '個人行程' : entry.type }}</em></span><small v-if="entry.location">{{ entry.location }}</small></span><span class="shopping-itinerary-picker-select">{{ shopping.itineraryItemId === entry.id ? '已選擇' : '選擇' }}</span></button></div>
</div>
<div v-else class="shopping-itinerary-picker-empty"><strong>尚未建立可關聯的行程</strong><p>先在每日行程中建立行程後，再回來關聯商品。</p></div>
<template #footer><el-button @click="showShoppingItineraryPicker=false">取消</el-button><el-button v-if="shopping.itineraryItemId" @click="clearShoppingItinerary">清除關聯</el-button></template>
  </el-dialog>
