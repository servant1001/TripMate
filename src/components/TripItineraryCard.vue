            ...
            <div
              ...
              <div v-show="!isCollapsed(entry)" class="itinerary-card-body">
                <template v-if="isItineraryGroup(entry)">
                  <div class="itinerary-group-summary"><p><strong>{{ entry.location || '未設定區域' }}</strong><span>共 {{ groupMembers(day, entry).length }} 項</span><span v-if="groupTypeSummary(day, entry)">{{ groupTypeSummary(day, entry) }}</span><span class="itinerary-group-visibility-copy">{{ groupVisibilitySummary(entry) }}</span></p><div class="itinerary-group-summary-actions"><a v-if="groupMapUrl(day, entry)" :href="groupMapUrl(day, entry)" target="_blank" rel="noopener noreferrer"><el-icon><Location /></el-icon>在 Google Maps 開啟 <el-icon><TopRight /></el-icon></a><a v-if="entry.website" :href="entry.website" target="_blank" rel="noopener noreferrer"><el-icon><Link /></el-icon>開啟網站 <el-icon><TopRight /></el-icon></a><el-button v-if="canEditTrip" class="itinerary-group-add-button" @click="emit('addGroupItem', entry)"><el-icon><Plus /></el-icon>新增底下行程</el-button></div></div>
