...
function itineraryTypeClass(type: string) {
  return ({ 景點: 'is-type-attraction', 餐廳: 'is-type-food', 交通: 'is-type-transport', 住宿: 'is-type-stay', 商店: 'is-type-shop' } as Record<string, string>)[type] || 'is-type-default'
}
        ...
        <div class="itinerary-list" :ref="(element) => registerSortableList(day.date, element as Element | null)">
          <article v-for="(entry, entryIndex) in day.entries" :key="entry.id" class="itinerary-entry" :class="{ 'is-completed': entry.completed, 'is-sortable-enabled': sortingEnabled && canEditTrip }">
            ...
            <div class="itinerary-card" :class="itineraryTypeClass(entry.type)">
              ...
              <div v-if="entry.type === '交通' && entry.transportDestinationName" class="itinerary-transport-route" aria-label="交通路線">
                <a v-if="entry.mapUrl || entry.location" class="itinerary-transport-stop is-linked" :href="mapsUrl(entry.location || entry.title, entry.mapUrl)" target="_blank" rel="noopener" :title="`在地圖中開啟出發站：${entry.location || entry.title}`"><span class="itinerary-transport-stop-label">出發</span><strong>{{ entry.location || entry.title }}</strong><el-icon><TopRight /></el-icon></a>
                <p v-else class="itinerary-transport-stop"><span class="itinerary-transport-stop-label">出發</span><strong>{{ entry.title }}</strong></p>
                <span class="itinerary-transport-arrow" aria-hidden="true">→</span>
                <a v-if="entry.transportDestinationMapUrl || entry.transportDestinationLocation" class="itinerary-transport-stop is-linked" :href="mapsUrl(entry.transportDestinationLocation || entry.transportDestinationName, entry.transportDestinationMapUrl)" target="_blank" rel="noopener" :title="`在地圖中開啟抵達站：${entry.transportDestinationLocation || entry.transportDestinationName}`"><span class="itinerary-transport-stop-label">抵達</span><strong>{{ entry.transportDestinationLocation || entry.transportDestinationName }}</strong><el-icon><TopRight /></el-icon></a>
                <p v-else class="itinerary-transport-stop"><span class="itinerary-transport-stop-label">抵達</span><strong>{{ entry.transportDestinationName }}</strong></p>
              </div>
              <template v-else><a v-if="entry.mapUrl || entry.location" class="itinerary-location is-linked" :href="mapsUrl(entry.location, entry.mapUrl)" target="_blank" rel="noopener" :title="`在地圖中開啟：${entry.location || entry.title}`"><el-icon><Location /></el-icon><span>{{ entry.location || '在 Google Maps 開啟' }}</span><el-icon class="itinerary-external-icon"><TopRight /></el-icon></a><p v-else class="itinerary-location is-empty"><el-icon><Location /></el-icon><span>尚未設定地點</span></p></template>
...
.itinerary-note{margin:6px 0;color:#6b7d78;font-size:13px;line-height:1.5;overflow-wrap:anywhere}.itinerary-transport-route{display:grid;grid-template-columns:minmax(0,1fr) 20px minmax(0,1fr);align-items:center;gap:4px;margin:1px 0 2px}.itinerary-transport-stop{display:grid;min-width:0;gap:2px;margin:0;padding:7px 8px;border:1px solid #d9e8e7;border-radius:9px;background:rgba(255,255,255,.6);color:#4f6964;line-height:1.35}.itinerary-transport-stop.is-linked{color:#276f65;text-decoration:none}.itinerary-transport-stop.is-linked:hover,.itinerary-transport-stop.is-linked:focus-visible{border-color:#86b9ae;background:#f7fcfa;outline:none}.itinerary-transport-stop.is-linked:hover strong{text-decoration:underline}.itinerary-transport-stop-label{color:#7c938d;font-size:10px;font-weight:800;letter-spacing:.5px}.itinerary-transport-stop strong{overflow:hidden;color:#285750;font-size:12px;line-height:1.4;overflow-wrap:anywhere}.itinerary-transport-stop .el-icon{position:absolute;right:6px;bottom:6px;font-size:11px}.itinerary-transport-stop.is-linked{position:relative;padding-right:20px}.itinerary-transport-arrow{display:grid;place-items:center;color:#45877c;font-size:18px;font-weight:800}.is-completed .itinerary-transport-route{opacity:.8}@media(max-width:720px){.itinerary-transport-route{grid-template-columns:minmax(0,1fr) 17px minmax(0,1fr);gap:3px}.itinerary-transport-stop{padding:6px 7px}.itinerary-transport-stop strong{font-size:11px}.itinerary-transport-arrow{font-size:16px}}@media(max-width:390px){.itinerary-transport-stop-label{font-size:9px}.itinerary-transport-stop strong{font-size:10px}}
.itinerary-card.is-type-shop{border-color:#ecd9aa;background:#fffaf0}.itinerary-type-chip.is-type-shop{background:#fff0cf;color:#8a621c}
</style>
