...
const props = defineProps<{ trip: Trip; items: ShoppingItem[]; exchangeRates?: Record<string, ExchangeRateQuote>; exchangeRateError?: string; canEditTrip: boolean; memberName: (memberId: string) => string }>()
const emit = defineEmits<{ add: []; edit: [item: ShoppingItem]; duplicate: [item: ShoppingItem]; remove: [item: ShoppingItem]; status: [item: ShoppingItem, status: ShoppingStatus]; convert: [item: ShoppingItem]; 'batch-link': [items: ShoppingItem[]] }>()
...
const statusLabels: Record<ShoppingStatus, string> = { wishlist: '想買', planned: '已規劃', purchased: '已購買', unavailable: '缺貨', cancelled: '已取消' }
const typeLabels: Record<ShoppingType, string> = { personal: '個人購物', proxy: '代購', shared: '共同採買', gift: '伴手禮' }
const priorityLabels: Record<ShoppingPriority, string> = { high: '高優先', medium: '一般', low: '低優先' }
const categories = computed(() => [...new Set(props.items.map((item) => item.category).filter(Boolean))].sort((a, b) => a.localeCompare(b, 'zh-Hant')))
const stores = computed(() => [...new Set(props.items.map((item) => item.storeName?.trim()).filter((store): store is string => Boolean(store)))].sort((a, b) => a.localeCompare(b, 'zh-Hant')))
const sortedItems = computed(() => [...props.items].sort((a, b) => {
  const statusOrder: Record<ShoppingStatus, number> = { planned: 0, wishlist: 1, unavailable: 2, cancelled: 3, purchased: 4 }
...
function unitAmountValue(item: ShoppingItem, actual = false) { return Math.max(0, Number(actual ? item.actualUnitPrice : item.estimatedUnitPrice) || 0) }
function showActualUnit(item: ShoppingItem) { return !!unitAmount(item, true) && (item.status === 'purchased' || !!unitAmount(item, false)) }
function showEstimatedUnit(item: ShoppingItem) { return !!unitAmount(item) && (item.status !== 'purchased' || !!unitAmount(item, true)) }
function unitLabel(item: ShoppingItem, actual = false) { const amount = unitAmount(item, actual); return amount ? `${actual ? '實際單價' : '預估單價'} ${amount} × ${item.quantity} ${item.unit || '件'}` : '' }
function taiwanPriceAmount(item: ShoppingItem) { const total = Math.max(0, Number(item.taiwanPrice) || 0); return total > 0 ? `TWD ${total.toLocaleString()}` : '' }
function comparisonBaseAmount(item: ShoppingItem) { return Math.max(0, Number(item.status === 'purchased' ? item.actualTotalPrice : item.estimatedTotalPrice) || ((Number(item.status === 'purchased' ? item.actualUnitPrice : item.estimatedUnitPrice) || 0) * item.quantity)) }
function currencyRate(item: ShoppingItem) { const code = item.currency?.trim().toUpperCase() || ''; return code === 'TWD' ? { from: 'TWD', to: 'TWD', rate: 1, date: new Date().toISOString().slice(0, 10), provider: 'TripMate local cache', cached: true } : props.exchangeRates?.[code] }
function convertedAmountTwd(item: ShoppingItem) { const base = comparisonBaseAmount(item); const quote = currencyRate(item); return base > 0 && quote?.rate ? Math.round(base * quote.rate * 100) / 100 : 0 }
function unitConvertedAmountTwd(item: ShoppingItem, actual = false) { const base = unitAmountValue(item, actual); const quote = currencyRate(item); return base > 0 && quote?.rate && item.currency?.trim().toUpperCase() !== 'TWD' ? Math.round(base * quote.rate * 100) / 100 : 0 }
function unitConvertedLabel(item: ShoppingItem, actual = false) { const total = unitConvertedAmountTwd(item, actual); return total > 0 ? `約 TWD ${numberLabel(total)}` : '' }
function exchangeRateDate(item: ShoppingItem) { return currencyRate(item)?.date || '' }
function exchangeRateLabel(item: ShoppingItem) { const quote = currencyRate(item); return quote && item.currency?.trim().toUpperCase() !== 'TWD' ? `1 ${quote.from} ≈ TWD ${quote.rate.toLocaleString(undefined, { minimumFractionDigits: quote.rate >= 1 ? 2 : 4, maximumFractionDigits: quote.rate >= 10 ? 2 : quote.rate >= 1 ? 3 : 4 })}` : '' }
function canCompareTaiwanPrice(item: ShoppingItem) { return Math.max(0, Number(item.taiwanPrice) || 0) > 0 && convertedAmountTwd(item) > 0 }
function taiwanCompareKind(item: ShoppingItem) { if (!canCompareTaiwanPrice(item)) return ''; const local = convertedAmountTwd(item); const taiwan = Math.max(0, Number(item.taiwanPrice) || 0); if (local < taiwan) return 'local-cheaper'; if (local > taiwan) return 'taiwan-cheaper'; return 'same' }
function taiwanCompareSummary(item: ShoppingItem) { const kind = taiwanCompareKind(item); if (kind === 'local-cheaper') return '當地較便宜'; if (kind === 'taiwan-cheaper') return '台灣較便宜'; if (kind === 'same') return '售價相同'; return '' }
...
function toggleSelectionMode() { selectionMode.value = !selectionMode.value; if (!selectionMode.value) selectedItemIds.value = [] }
function toggleItemSelection(item: ShoppingItem, checked: boolean) { const selected = new Set(selectedItemIds.value); if (checked) selected.add(item.id); else selected.delete(item.id); selectedItemIds.value = [...selected] }
function linkSelectedItems() { const selected = props.items.filter((item) => selectedItemIds.value.includes(item.id)); if (!selected.length) return; emit('batch-link', selected); selectionMode.value = false; selectedItemIds.value = [] }
function primaryAmount(item: ShoppingItem) { return amount(item, item.status === 'purchased') || amount(item) }
function primaryAmountLabel(item: ShoppingItem) { return item.status === 'purchased' ? '實際' : amount(item) ? '預估' : '' }
</script>
...
<template>
  <section id="shopping" class="trip-detail-card shopping-panel">
    ...
    <div v-if="visibleItems.length" class="shopping-groups" :class="{ 'is-store-mode': storeMode }">
      <section v-for="group in displayGroups" :key="group.name || 'all'" class="shopping-store-group">
        <div v-if="groupByStore" class="shopping-store-heading"><div><el-icon><Location /></el-icon><strong>{{ group.name }}</strong></div><span>{{ group.items.filter((item) => item.status !== 'purchased').length }} 件待購買</span></div>
        <div class="shopping-list">
          <article v-for="item in group.items" :key="item.id" class="shopping-row" :class="[`is-${item.status}`, `is-${item.priority}`, { 'is-selecting': selectionMode }]">
            <el-checkbox v-if="selectionMode" class="shopping-select-checkbox" :model-value="selectedItemIds.includes(item.id)" :aria-label="`選取 ${item.name}`" @change="(checked: boolean) => toggleItemSelection(item, checked)" />
            ...
            <div class="shopping-copy"><div class="shopping-title"><strong>{{ item.name }}</strong><span :class="['shopping-status-badge', `is-${item.status}`]">{{ statusLabels[item.status] }}</span><span :class="['shopping-priority-badge', `is-${item.priority}`]">{{ priorityLabels[item.priority] }}</span></div><p class="shopping-tags"><span>{{ typeLabels[item.shoppingType] }}</span><span>{{ item.category }}</span><span v-if="item.quantity > 1">{{ item.quantity }} {{ item.unit || '件' }}</span></p><p v-if="item.storeName || item.assignedTo || item.requestedBy || item.giftRecipient" class="shopping-meta"><span v-if="item.storeName"><el-icon><Location /></el-icon>{{ item.storeName }}{{ item.storeBranch ? `・${item.storeBranch}` : '' }}</span><span v-if="item.assignedTo"><el-icon><User /></el-icon>{{ memberName(item.assignedTo) }} 負責</span><span v-if="item.requestedBy">委託：{{ item.requestedBy }}</span><span v-if="item.giftRecipient">送給：{{ item.giftRecipient }}</span></p><p v-if="item.note" class="shopping-note">{{ item.note }}</p></div>
            <div class="shopping-price">
              <div
                v-if="primaryAmount(item)"
                class="shopping-price-main"
                :class="item.status === 'purchased' ? 'is-actual' : 'is-estimated'"
              >
              ...
              <small v-else-if="needsTaiwanCompareHint(item)" class="shopping-compare-hint">{{ exchangeRateError || `尚未取得 ${item.currency} → TWD 最新匯率` }}</small>
            </div>
            <div class="shopping-tools">
              <el-tooltip v-if="mapsUrl(item)" content="在 Google Maps 開啟店家" placement="top"><a class="shopping-tool-button" :href="mapsUrl(item)" target="_blank" rel="noopener noreferrer" aria-label="在 Google Maps 開啟店家"><el-icon><TopRight /></el-icon></a></el-tooltip>
              <el-tooltip v-if="canEditTrip && item.status !== 'purchased' && item.status !== 'cancelled'" content="標記為已購買" placement="top"><el-button class="shopping-tool-button is-purchase" text circle :aria-label="`標記 ${item.name} 為已購買`" @click="emit('status', item, 'purchased')">✓</el-button></el-tooltip>
              <el-dropdown v-if="canEditTrip" trigger="click" @command="(status: ShoppingStatus) => emit('status', item, status)"><el-button class="shopping-tool-button is-status" text circle aria-label="變更商品狀態" title="變更商品狀態"><el-icon><Switch /></el-icon></el-button><template #dropdown><el-dropdown-menu><el-dropdown-item v-for="(label, status) in statusLabels" :key="status" :command="status" :disabled="item.status === status">{{ label }}</el-dropdown-item></el-dropdown-menu></template></el-dropdown>
              <el-tooltip v-if="canEditTrip && item.status === 'purchased' && !item.expenseId" content="建立旅行開銷" placement="top"><el-button class="shopping-tool-button is-expense" text circle aria-label="建立旅行開銷" @click="emit('convert', item)">¥</el-button></el-tooltip>
              <el-dropdown v-if="canEditTrip" trigger="click" @command="(command: 'edit' | 'duplicate' | 'remove') => handleItemAction(command, item)"><el-button class="shopping-tool-button is-more" text circle aria-label="更多商品操作" title="更多商品操作"><el-icon><MoreFilled /></el-icon></el-button><template #dropdown><el-dropdown-menu><el-dropdown-item command="edit">編輯商品</el-dropdown-item><el-dropdown-item command="duplicate">複製商品</el-dropdown-item><el-dropdown-item command="remove" divided class="shopping-delete-action">刪除商品</el-dropdown-item></el-dropdown-menu></template></el-dropdown>
            </div>
          </article>
        </div>
      </section>
    </div>
    <div v-else class="detail-empty-state shopping-empty"><el-icon><Picture /></el-icon><div><strong>{{ items.length ? '沒有符合篩選條件的商品' : '還沒有購物項目' }}</strong><p>{{ items.length ? '調整篩選條件，或新增新的購物項目。' : '把想買的伴手禮、代購或共同採買商品先記下來。' }}</p></div><el-button v-if="canEditTrip" class="shopping-add-button" @click="emit('add')">新增第一個商品</el-button></div>
  </section>
</template>
...
.shopping-add-button,.shopping-store-mode{display:inline-flex;gap:6px;min-height:40px;border-radius:10px;font-weight:700}
...
.shopping-store-mode{border-color:#c8ded4;background:#fff;color:#2f7d70}
...
.shopping-tool-button.is-danger{color:#c36358}
.shopping-tool-button.is-danger:hover,.shopping-tool-button.is-danger:focus-visible{background:#fdf0ed;color:#b64237}
...
@media(max-width:600px){
  ...
  .shopping-store-mode{font-size:12px}
