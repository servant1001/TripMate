  ...
  <section id="expenses" class="trip-detail-card expense-panel">
    ...
    <section class="daily-budget-card" :class="{ 'is-unset': !dailyBudget }">
      ...
      <template v-if="dailyBudget">
        <article v-for="day in dailyExpenses" :key="day.date" class="daily-budget-row" :class="{ 'is-over': day.spent > dailyBudget }"><div><strong>{{ day.date }}</strong><span>{{ trip.currency }} {{ day.spent.toLocaleString() }}／{{ dailyBudget.toLocaleString() }}</span></div><b>{{ budgetPercentLabel(day.spent, dailyBudget) }}</b><span class="daily-budget-track"><i :style="{ width: `${budgetRatio(day.spent, dailyBudget) * 100}%` }"></i></span></article>
