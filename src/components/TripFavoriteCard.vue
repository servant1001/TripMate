...
function typeLabel(type: FavoriteType) { return ({ attraction: '景點', restaurant: '餐廳', transport: '交通', stay: '住宿', shop: '景點', cafe: '餐廳', alternative: '景點', other: '景點' } as Record<FavoriteType, string>)[type] }
...
function normalizedType(type: FavoriteType): FavoriteType { return type === 'cafe' ? 'restaurant' : type === 'shop' || type === 'alternative' || type === 'other' ? 'attraction' : type }
...
const filterOptions: Array<{ value: FavoriteType | 'all'; label: string }> = [
  { value: 'all', label: '全部' }, { value: 'attraction', label: '景點' }, { value: 'restaurant', label: '餐廳' }, { value: 'transport', label: '交通' }, { value: 'stay', label: '住宿' },
]
...
const visibleFavorites = computed(() => activeFilter.value === 'all' ? sortedFavorites.value : sortedFavorites.value.filter((favorite) => normalizedType(favorite.type) === activeFilter.value))
...
<template>
  <section id="favorites" class="trip-detail-card favorite-panel">
    ...
    <div v-else-if="favorites.length" class="favorite-no-results"><el-icon><MapLocation /></el-icon><strong>這個類別還沒有收藏</strong><p>試試切換其他類別，或新增一筆 {{ activeFilter === 'all' ? '' : typeLabel(activeFilter) }} 收藏。</p></div>
