...
import { House, Location, MapLocation, MoreFilled, Mug, Rank, Search, ShoppingBag, TopRight } from '@element-plus/icons-vue'
...
function matchesKeyword(favorite: Favorite) {
  ...
  return [
    ...
    favorite.location,
    favorite.note,
    favorite.recommendedBy,
    favorite.website,
    favorite.mapUrl,
    props.memberName(favorite.createdBy),
    favorite.addedToItinerary ? '已加入行程' : '尚未加入行程',
...
const visibleFavorites = computed(() => sortedFavorites.value.filter((favorite) => {
  ...
  return matchesType && matchesCreator && matchesItinerary && matchesKeyword(favorite)
}))
...
<template>
  <section id="favorites" class="trip-detail-card favorite-panel">
    ...
    <div v-if="favorites.length" class="favorite-filter-wrap">
      <label class="favorite-search-bar">
        <span class="favorite-search-label">搜尋收藏</span>
        <el-input v-model="keywordFilter" :disabled="sortingEnabled" placeholder="搜尋名稱、地點、類型、建立人或備註" clearable aria-label="搜尋旅遊收藏">
    ...
    <div v-else-if="favorites.length" class="favorite-no-results"><el-icon><MapLocation /></el-icon><strong>沒有符合條件的收藏</strong><p>{{ keywordFilter.trim() ? '可以調整搜尋關鍵字，或搭配類別、建立人與行程狀態重新篩選。' : '試試切換類別、建立人或行程狀態，或新增一筆收藏。' }}</p></div>
...
.favorite-filter-wrap{display:grid;gap:8px}.favorite-search-bar{display:grid;gap:6px}.favorite-search-label{color:#6b7d78;font-size:12px;font-weight:700}.favorite-search-bar :deep(.el-input__wrapper){min-height:42px;border-radius:10px}.favorite-filter-selects{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.favorite-filter-select{display:grid;min-width:0;gap:4px;color:#6b7d78;font-size:12px;font-weight:700}.favorite-filter-select .el-select{width:100%}.favorite-filter-select :deep(.el-select__wrapper){min-height:38px;border-radius:9px}@media(max-width:720px){.favorite-filter-selects{grid-template-columns:1fr}}
