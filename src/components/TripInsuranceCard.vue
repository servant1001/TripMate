...
const props = defineProps<{ trip: Trip; insurances: TravelInsurance[]; statuses: Record<string, InsuranceStatusSummary>; userId: string; memberName: (id: string) => string; canEdit: boolean; saving?: boolean }>()
const emit = defineEmits<{ save: [payload: InsuranceFormPayload, files: File[]]; remove: [insurance: TravelInsurance]; openAttachment: [attachment: InsuranceAttachment] }>()
...
const sortedPolicies = computed(() => [...props.insurances].sort((left, right) => (right.updatedAt || right.createdAt || 0) - (left.updatedAt || left.createdAt || 0)))
const editingInsurance = computed(() => sortedPolicies.value.find((item) => item.id === editingInsuranceId.value))
...
function resetForm(source?: TravelInsurance) {
  ...
  form.policyNumber = source?.policyNumber || ''
  form.insuredName = source?.insuredName || props.memberName(props.userId)
  form.coverageStart = toInputDate(source?.coverageStartAt)
  form.coverageEnd = toInputDate(source?.coverageEndAt)
  ...
  form.assistancePhone = source?.assistancePhone || ''
  form.website = source?.website || ''
  form.note = source?.note || ''
  form.visibility = source?.visibility || 'private'
  form.status = source?.status || 'draft'
  form.attachments = [...(source?.attachments || [])]
  uploadFiles.value = []
}

function openForm(source?: TravelInsurance) {
  editingInsuranceId.value = source?.id || null
  resetForm(source)
  showForm.value = true
}
...
function onFiles(event: Event) {
  ...
  uploadFiles.value.push(...files.filter((file) => file.size <= 10 * 1024 * 1024 && /^(image\/(jpeg|png|webp)|application\/pdf)$/.test(file.type)))
}
...
function save() {
  if (!form.providerName.trim() || !form.policyName.trim() || !form.insuredName.trim() || !form.coverageStart || !form.coverageEnd) return
  ...
  emit('save', {
    ...
    assistancePhone: form.assistancePhone.trim() || undefined,
...
async function removePolicy(insurance: TravelInsurance) {
  try {
    await ElMessageBox.confirm('確定要刪除這張保單嗎？附件也會從資料庫移除。', '刪除保單', {
      type: 'warning',
      confirmButtonText: '刪除',
      cancelButtonText: '取消',
    })
    emit('remove', insurance)
  } catch {
    /* cancelled */
  }
}

watch(() => props.insurances, () => {
  if (editingInsuranceId.value && !props.insurances.some((item) => item.id === editingInsuranceId.value)) {
  ...
  <section class="insurance-card panel">
    <header class="insurance-heading">
      <div>
        <p>INSURANCE</p>
        <h2>旅行保險與緊急資訊</h2>
        <span>集中保管自己的保單、保障期間與緊急聯絡方式。現在可同時建立多張保單，方便記錄不同保險公司方案。</span>
      </div>
      <el-button v-if="canEdit" type="primary" class="insurance-add" @click="openCreateForm">＋ 新增保單</el-button>
    </header>

    <div class="insurance-summary">
      <article><span>旅行成員</span><strong>{{ trip.members.length }} 人</strong></article>
      <article><span>已提供保單</span><strong>{{ Object.keys(statuses).length }} 人</strong></article>
      <article><span>我的保單</span><strong>{{ sortedPolicies.length }} 張</strong></article>
      <article><span>保障完整</span><strong class="warning">{{ activeCount }} 人</strong></article>
    </div>

    <div v-if="sortedPolicies.length" class="insurance-policy-list">
      <article v-for="policy in sortedPolicies" :key="policy.id" class="insurance-policy">
        <div class="insurance-policy-top">
          <div>
            <span :class="['insurance-status', `is-${coverageFor(policy)}`]">{{ coverageLabel[coverageFor(policy)] }}</span>
            <h3>{{ policy.policyName }}</h3>
            <p>{{ policy.providerName }} · {{ policy.insuredName }}</p>
          </div>
          <el-dropdown v-if="canEdit" trigger="click">
            <button class="insurance-more" type="button" aria-label="保單更多操作">⋯</button>
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item @click="openForm(policy)">編輯保單</el-dropdown-item>
                <el-dropdown-item divided class="danger-action" @click="removePolicy(policy)">刪除保單</el-dropdown-item>
              </el-dropdown-menu>
            </template>
          </el-dropdown>
        </div>
        ...
        <div v-if="policy.emergencyPhone || policy.customerServicePhone || policy.claimPhone || policy.assistancePhone" class="insurance-phones">
          <a v-if="policy.emergencyPhone" :href="`tel:${policy.emergencyPhone}`">緊急救援 {{ policy.emergencyPhone }}</a>
          <a v-if="policy.customerServicePhone" :href="`tel:${policy.customerServicePhone}`">客服 {{ policy.customerServicePhone }}</a>
    ...
    <div v-else class="insurance-empty">
      <span>🛡</span>
      <strong>尚未建立你的保險資料</strong>
      <p>保單與緊急聯絡資訊只會依你的可見性設定分享。</p>
      <el-button v-if="canEdit" @click="openCreateForm">新增我的保單</el-button>
    </div>

    <div class="insurance-members">
      <h3>成員保險狀態</h3>
      <div v-for="row in memberRows" :key="row.member.id" class="insurance-member">
        <span class="insurance-avatar">{{ row.member.name.slice(0, 1) }}</span>
        <div>
          <strong>{{ row.member.name }} <small v-if="row.mine">（我）</small></strong>
          <span>{{ row.summary?.providerName || (row.summary ? '已提供保單' : '尚未提供') }}</span>
        </div>
        <b :class="`member-status is-${row.summary?.status || 'not_provided'}`">
          {{ row.summary?.status === 'covered' ? '保障完整' : row.summary?.status === 'coverage_gap' ? '保障不足' : row.summary?.status === 'draft' ? '草稿' : row.summary?.status === 'expired' ? '已過期' : row.summary?.status === 'cancelled' ? '已取消' : '尚未提供' }}
        </b>
      </div>
    </div>
  </section>

  <el-dialog v-model="showForm" class="insurance-dialog" :title="editingInsurance ? '編輯保單' : '新增保單'" width="min(94vw, 760px)" destroy-on-close>
    <el-form label-position="top">
      <div class="insurance-form-grid">
        <el-form-item label="保險公司" required><el-input v-model="form.providerName" placeholder="例如：國泰產險" /></el-form-item>
        <el-form-item label="保單名稱" required><el-input v-model="form.policyName" placeholder="例如：海外旅遊綜合保險" /></el-form-item>
        <el-form-item label="保單號碼（選填）"><el-input v-model="form.policyNumber" /></el-form-item>
        <el-form-item label="被保險人" required><el-input v-model="form.insuredName" /></el-form-item>
        <el-form-item label="保費（選填）"><el-input-number v-model="form.premiumAmount" :min="0" controls-position="right" /></el-form-item>
        <el-form-item label="幣別"><el-select v-model="form.premiumCurrency"><el-option :label="trip.currency" :value="trip.currency" /><el-option label="TWD" value="TWD" /><el-option label="JPY" value="JPY" /><el-option label="USD" value="USD" /></el-select></el-form-item>
        <el-form-item label="保障開始" required><el-date-picker v-model="form.coverageStart" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" /></el-form-item>
        <el-form-item label="保障結束" required><el-date-picker v-model="form.coverageEnd" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" /></el-form-item>
      </div>
      ...
      <div class="insurance-form-grid">
        <el-form-item label="緊急救援電話"><el-input v-model="form.emergencyPhone" inputmode="tel" /></el-form-item>
        <el-form-item label="客服電話"><el-input v-model="form.customerServicePhone" inputmode="tel" /></el-form-item>
        <el-form-item label="理賠電話"><el-input v-model="form.claimPhone" inputmode="tel" /></el-form-item>
        <el-form-item label="旅遊協助電話"><el-input v-model="form.assistancePhone" inputmode="tel" /></el-form-item>
      </div>
      <el-form-item label="保險網站"><el-input v-model="form.website" placeholder="https://" /></el-form-item>
      <el-form-item label="備註"><el-input v-model="form.note" type="textarea" maxlength="500" show-word-limit /></el-form-item>
      <el-form-item label="保單附件（JPG、PNG、WebP、PDF；單檔 10 MB）">
        <input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" multiple @change="onFiles" />
        <div v-if="form.attachments.length || uploadFiles.length" class="insurance-file-list">
          <span v-for="(attachment, index) in form.attachments" :key="attachment.secureUrl">{{ attachment.originalFilename || '已上傳附件' }} <button type="button" @click="removeAttachment(index)">移除</button></span>
          <span v-for="(file, index) in uploadFiles" :key="`${file.name}-${index}`">{{ file.name }} <button type="button" @click="removePending(index)">移除</button></span>
        </div>
      </el-form-item>
    </el-form>
