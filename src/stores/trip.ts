...
import type { AlbumPhoto, Booking, Expense, Favorite, ItineraryItem, Member, PackingItem, Settlement, TodoItem, Trip } from '../types'
import { repository } from '../services/repository'

export const useTripStore = defineStore('trips', {
  state: () => ({ trips: [] as Trip[], itinerary: [] as ItineraryItem[], expenses: [] as Expense[], settlements: [] as Settlement[], todos: [] as TodoItem[], packingItems: [] as PackingItem[], bookings: [] as Booking[], favorites: [] as Favorite[], albumPhotos: [] as AlbumPhoto[], categoryBudgets: {} as Record<string, Record<string, number>>, loading: false }),
  getters: { trip: (s) => (id: string) => s.trips.find((x) => x.id === id), items: (s) => (id: string) => s.itinerary.filter((x) => x.tripId === id), tripExpenses: (s) => (id: string) => s.expenses.filter((x) => x.tripId === id), tripSettlements: (s) => (id: string) => s.settlements.filter((x) => x.tripId === id), tripTodos: (s) => (id: string) => s.todos.filter((x) => x.tripId === id), tripPackingItems: (s) => (id: string) => s.packingItems.filter((x) => x.tripId === id), tripBookings: (s) => (id: string) => s.bookings.filter((x) => x.tripId === id), tripFavorites: (s) => (id: string) => s.favorites.filter((x) => x.tripId === id), tripAlbumPhotos: (s) => (id: string) => s.albumPhotos.filter((x) => x.tripId === id), tripCategoryBudgets: (s) => (id: string) => s.categoryBudgets[id] || {} },
  actions: {
    ...
    async deleteAlbumPhoto(photo: AlbumPhoto) { await repository.deleteAlbumPhoto(photo); this.albumPhotos = this.albumPhotos.filter((entry) => entry.id !== photo.id) },
