...
import type { Booking, Expense, ItineraryItem, Member, PackingItem, Settlement, TodoItem, Trip } from '../types'
import { repository } from '../services/repository'

export const useTripStore = defineStore('trips', {
  state: () => ({ trips: [] as Trip[], itinerary: [] as ItineraryItem[], expenses: [] as Expense[], settlements: [] as Settlement[], todos: [] as TodoItem[], packingItems: [] as PackingItem[], bookings: [] as Booking[], categoryBudgets: {} as Record<string, Record<string, number>>, loading: false }),
  getters: { trip: (s) => (id: string) => s.trips.find((x) => x.id === id), items: (s) => (id: string) => s.itinerary.filter((x) => x.tripId === id), tripExpenses: (s) => (id: string) => s.expenses.filter((x) => x.tripId === id), tripSettlements: (s) => (id: string) => s.settlements.filter((x) => x.tripId === id), tripTodos: (s) => (id: string) => s.todos.filter((x) => x.tripId === id), tripPackingItems: (s) => (id: string) => s.packingItems.filter((x) => x.tripId === id), tripBookings: (s) => (id: string) => s.bookings.filter((x) => x.tripId === id), tripCategoryBudgets: (s) => (id: string) => s.categoryBudgets[id] || {} },
  actions: {
    ...
    async deleteTrip(trip: Trip) { await repository.deleteTrip(trip); this.trips = this.trips.filter((item) => item.id !== trip.id); this.itinerary = this.itinerary.filter((item) => item.tripId !== trip.id); this.expenses = this.expenses.filter((item) => item.tripId !== trip.id); this.settlements = this.settlements.filter((item) => item.tripId !== trip.id); this.todos = this.todos.filter((item) => item.tripId !== trip.id); this.packingItems = this.packingItems.filter((item) => item.tripId !== trip.id); this.bookings = this.bookings.filter((item) => item.tripId !== trip.id); delete this.categoryBudgets[trip.id] },
