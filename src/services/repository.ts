...
export const repository = {
  async getData(userId?: string) {
    ...
    const tripRows = await Promise.all(ids.map(async (tripId) => {
      const [tripSnapshot, membersSnapshot, itinerarySnapshot, expensesSnapshot, settlementsSnapshot, personalBudgetSnapshot] = await Promise.all([
        get(ref(db, `trips/${tripId}`)), get(ref(db, `tripMembers/${tripId}`)),
        get(ref(db, `itineraryItems/${tripId}`)), get(ref(db, `expenses/${tripId}`)), get(ref(db, `settlements/${tripId}`)), get(ref(db, `tripMemberBudgets/${tripId}/${userId}`)),
      ])
