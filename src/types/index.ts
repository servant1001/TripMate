...
export type ExpenseSplitMode = 'equal' | 'custom' | 'ratio' | 'shares'
...
/** `payerId` and `shares` are retained for compatibility with existing expenses. */
export interface Expense { id: string; tripId: string; title: string; amount: number; payerId: string; payerShares?: Record<string, number>; kind: ExpenseKind; participantIds: string[]; splitMode?: ExpenseSplitMode; shares?: Record<string, number>; ratios?: Record<string, number>; splitUnits?: Record<string, number>; category: string; date: string; sourceCurrency?: string; sourceAmount?: number; exchangeRate?: number; receiptUrl?: string; note?: string }
